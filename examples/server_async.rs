...
async fn main() -> Result<()> {
    ...
    loop {
        ...
        println!("incomming connection from {addr:?}");
        tokio::spawn(async {
...
async fn handle_client(mut stream: TcpStream) -> Result<()> {
        ...
        read_header(&mut stream, &mut read_buf, &mut read_cursor).await?
    {
        ...
        framer
            .accept(&mut stream, &mut frame_buf, &websocket_context)
            .await?;
        println!("Websocket connection opened");

        // read websocket frames
        while let Some(read_result) = framer.read(&mut stream, &mut frame_buf).await {
